...
type EditCandidateExperienceRouteProp = RouteProp<
  RootStackParamList,
...
export function EditCandidateExperience() {
  ...
  useEffect(() => {
    ...
    if (selectedExperience) {
      ...
      setStartDate(new Date(selectedExperience.start_date));
      setEndDate(new Date(selectedExperience.end_date));
      setisCurrently(selectedExperience.currently);
    }
  }, [selectedExperience]);
  ...
  const handleAddExperience = () => {
    if (experiences.length >= 5) {
      Alert.alert(
        "Limite Atingido",
        "Você já atingiu o limite de 5 experiências."
      );
      return;
    }
  ...
  const handleUpdateExperience = () => {
    ...
    const updatedExperiences = experiences.map((exp) =>
      exp.candidate_experience_id === selectedExperience.candidate_experience_id
        ? updatedExperience
        : exp
    );
  ...
  const handleRemoveExperience = (candidate_experience_id: string) => {
    ...
    const experienceToRemove = experiences.find(
      (exp) => exp.candidate_experience_id === candidate_experience_id
    );
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              await api.delete(
                `/candidate/experience/${candidate_experience_id}`
              );
                ...
                <View style={styles.editContainer}>
                  ...
                  >
                    <View style={styles.checkbox}>
                      {currently && <View style={styles.checked} />}
                    </View>

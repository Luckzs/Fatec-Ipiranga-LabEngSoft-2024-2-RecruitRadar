...
export function ProfileScreen() {
    ...
    Alert.alert("Confirmação", "Você tem certeza que deseja sair?", [
      ...
      },
      {
        ...
        onPress: () => {
          ...
          console.log(
            `Após limpar, vacancyData possui ${vacancyData.length} vagas.`
          );

          // Aguarda um tempo curto para garantir que o estado seja atualizado
          setTimeout(() => {
            console.log(
              `Após limpar, vacancyData possui ${vacancyData.length} vagas.`
            );
            signOut(); // Chama signOut após limpar vacancyData
          }, 100);
        },
        style: "destructive",
      },
  ...
  useEffect(() => {
    // Define um temporizador para mostrar o botão após 1 minuto (60000ms)
      ...
      <View style={styles.loadingContainer}>
        ...
        {showButton && (
          <View style={styles.refreshButtonContainer}>
            <TouchableOpacity
              style={styles.refreshButton1}
              onPress={handleSignOut}
            >
              <Text style={styles.refreshButtonText}>Sair</Text>
            </TouchableOpacity>
          </View>
        )}
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>Área Profissional</Text>
        <View style={styles.pereferencessection}>
          {/* Exibir Cargos */}
          {profileData.candidateObjectives.map((obj) => (
            <View key={obj.candidate_objective_id} style={styles.studyItem}>
              <Text style={styles.studyText}>{obj.professional_area}</Text>
            </View>
          ))}
        </View>

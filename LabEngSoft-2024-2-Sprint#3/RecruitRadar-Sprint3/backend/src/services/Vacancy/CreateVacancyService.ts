...
class CreateVacancyService {
    async execute(data: IVacancyRequest) {
        ...
        const {
            ...
            language_course_name: data.language_course_name,
        ...
        //TODO:ENTENDER PQ ESTA INSERINDO FORMAÇÕES QUE APARENTEMENTE TEM O MESMO NOME, REPETINDO ASSIM
        ...
        if (language_course_name && language_course_name.length > 0) {
            for (let i = 0; i < language_course_name.length; i++) {
                let languageName = normalizeString(language_course_name[i]); // Normalizando a string 

                if (languageName && languageName.toLowerCase() !== "não informado") {
                    let languageLevel = "Básico";

                    const regex = /([^\(\)]+)(?:\(([^)]+)\))?/;
                    const match = regex.exec(languageName);
